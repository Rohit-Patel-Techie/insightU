...
import {
  ...
  OptionGrid,
  Question,
} from "@/components/check-in/CheckInPrimitives";
import {
  dayTypeOptions,
  distractionOptions,
...
export function StudyProgressSection({ form, errors, setField, className }) {
  const hoursLabel =
    form.studyHours === 8 ? "8+" : Number(form.studyHours).toFixed(1);
  return (
    <CheckInCard
      step={1}
      title="Study Progress"
      helper="Let's see how your study day went."
      emoji="📚"
      className={className}
    >
      ...
      <Question label="How many hours did you study today?">
        <div className="rounded-2xl bg-slate-50 p-4 dark:bg-white/[0.035]">
          <div className="flex items-end justify-between gap-3">
            <span className="font-display text-3xl font-extrabold text-indigo-600 dark:text-indigo-400">
              {hoursLabel}
            </span>
            <span className="pb-1 text-xs font-bold text-slate-400">Hours</span>
          </div>
          <input
            className="checkin-range mt-4 w-full"
            type="range"
            min="0"
            max="8"
            step="0.5"
            ...
            onChange={(event) =>
              setField("studyHours", Number(event.target.value))
            }
      ...
      <Question label="Did you complete your planned study today?">
        <OptionGrid
          ...
          onChange={(value) => setField("studyCompletion", value)}
          columns={3}
          ariaLabel="Planned study completion"
        />
      ...
      <Question label="How focused were you today?">
        <FocusScale
          ...
          onChange={(value) => setField("focusLevel", value)}
        />
...
export function MoodEnergySection({ form, errors, setField, className }) {
  return (
    <CheckInCard
      step={2}
      title="Mood & Energy"
      helper="Your mood matters. Be honest with yourself."
      emoji="😊"
      className={className}
    >
      ...
      <Question label="What best describes your day?">
        <OptionGrid
          ...
          value={form.dayType}
          onChange={(value) => setField("dayType", value)}
          columns={3}
          ariaLabel="Day type"
        />
    ...
    >
      ...
      >
        <OptionGrid
          ...
          onChange={(value) => toggleListValue("distractions", value)}
          columns={3}
          multiple
      ...
      >
        <div
          className={cn(selectedNothing && "pointer-events-none opacity-45")}
        >
          <OptionGrid
            options={distractionTimeOptions}
            value={form.distractionTime}
            onChange={(value) => setField("distractionTime", value)}
            columns={4}
            ariaLabel="Distraction time"
          />
        </div>
...
export function ReflectionSection({ form, setField, className }) {
  return (
    <CheckInCard
      step={5}
      title="Reflection"
      helper="A quick reflection helps you improve."
      emoji="✨"
      className={className}
    >
      <ReflectionField
        ...
        value={form.wentWell}
        onChange={(value) => setField("wentWell", value)}
        placeholder="For example: I completed my assignment and stayed focused..."
      />
      <ReflectionField
        label="What will you improve tomorrow?"
        value={form.improveTomorrow}
        onChange={(value) => setField("improveTomorrow", value)}
        placeholder="For example: I will avoid social media during study time..."
      />
    </CheckInCard>
  );
}
...
export function SummarySection({
  ...
  const summary = [
    ...
    [
      "Top Distraction",
      form.distractions
        .map((value) => optionLabel(distractionOptions, value))
        .slice(0, 2)
        .join(", ") || "Not selected",
      "📱",
    ],
    [
      "Distraction Time",
      form.distractions.includes("nothing")
        ? "Not applicable"
        : optionLabel(distractionTimeOptions, form.distractionTime),
      "🕐",
    ],
  ];
  return (
    <CheckInCard
      step={6}
      title="Summary"
      helper="Review today's check-in before submitting."
      emoji="🎉"
      className={className}
    >
      {submitted && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-extrabold text-emerald-900 dark:text-emerald-200">
                Check-in saved, {user.firstName}!
              </p>
              <p className="mt-1 text-xs leading-5 text-emerald-700 dark:text-emerald-300">
                Your daily check-in was saved successfully and is now available
                to your account.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {summary.map(([label, value, emoji]) => (
          <div
            key={label}
            className="min-w-0 rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-white/[0.06] dark:bg-white/[0.035]"
          >
            ...
            <p className="mt-1 break-words text-xs font-extrabold leading-4 text-slate-800 dark:text-slate-100">
              {value}
            </p>
          </div>
        ))}
